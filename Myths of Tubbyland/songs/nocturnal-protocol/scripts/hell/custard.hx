...
// Dynamic scaling value for 15 custards (keeps precision flawless)
var riskStep:Float = 1 / 15; 
...
function onNoteHit(e) {
    if(e.noteType == "custard") {
        ...
        // Each hit reduces the risk by 1/15th (~6.66%)
        deathChance -= riskStep;
        if (deathChance < 0) deathChance = 0;
        
        custardCounter.color = 0xFFFFFFFF;
        deathChanceText.color = 0xFFFFFFFF;
        custardCounter.text = "Custards Collected: " + custardsCollected + "/15"; // Scaled to /15
        ...
        // Safeguard to make sure displaying 0% + 0% doesn't round weirdly to 101% due to floats
        if (displayPercent + penaltyPercent > 100) displayPercent = 100 - penaltyPercent;
...
function onPlayerMiss(e) {
    if(e.noteType == "custard") {
        ...
        // Each miss locks away 1/15th (~6.66%) permanently 
        permanentDeathChance += riskStep;
        deathChance -= riskStep;
        if (deathChance < 0) deathChance = 0;
...
function borderOFF() {
    custardCounter = new FlxText(20, 20, 0, "Custards Collected: 0/15", 48); // Scaled to /15
    setupText(custardCounter);
    add(custardCounter);
}

function borderPartial() {
    custardCounter = new FlxText(((FlxG.width - 960) / 2) + 20, 20, 0, "Custards Collected: 0/15", 48); // Scaled to /15
    setupText(custardCounter);
    add(custardCounter);
}

function borderON() {
    custardCounter = new FlxText(((FlxG.width - 390) / 2) + 20, healthBar.y - 150, 0, "Custards Collected: 0/15", 48); // Scaled to /15
    setupText(custardCounter);
    add(custardCounter);
}