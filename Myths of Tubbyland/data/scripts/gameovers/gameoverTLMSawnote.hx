...
function create(event) {
    ...
    var deathSound:FlxSound = FlxG.sound.play(Paths.sound("gameover/deathTLM"));

    // Set up the background image
    deathImg = new FlxSprite(0, 0).loadGraphic(Paths.image("gameover/deathTLM"));
    ...
    infoText = new FlxText(0, FlxG.height * 0.5, FlxG.width, "BLOCK DIPSY'S CHAINSAW", 48);
    infoText.setFormat(Paths.font(fontName), 48, 0xFF46FF46, "center");
    infoText.scrollFactor.set(0, 0);
    infoText.alpha = 0;
    add(infoText);

    // Set up the hint text at the bottom
    hintText = new FlxText(0, FlxG.height - 50, FlxG.width, "Not hitting Saw Notes will increase the damage you take!", 24);
